      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <Card className="p-5 lg:col-span-2">
          ...
          <div className="mt-5 flex justify-end gap-2">
            <Button className="bg-gradient-eco" onClick={() => setConfirmOpen(true)} disabled={loading}>
          ...
          <Dialog open={confirmOpen} onOpenChange={(open) => setConfirmOpen(open)}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Confirma la creación de la solicitud</DialogTitle>
                <DialogDescription>
                  Revisa cuidadosamente la información: una vez creada la solicitud en "Mis solicitudes" no podrá ser modificada.
                </DialogDescription>
              </DialogHeader>
              <div className="space-y-2 mt-4">
                <div><strong>Litros estimados:</strong> {volumen} L</div>
                <div><strong>Fecha disponible:</strong> {fecha || "Sin definir"}</div>
                <div><strong>Precio por litro:</strong> S/ {Number(precioOfertado).toFixed(2)}</div>
                <div><strong>Dirección:</strong> {direccion}</div>
                <div><strong>Observaciones:</strong> {observaciones || "-"}</div>
                <div className="mt-2 font-semibold">Monto estimado: S/ {montoEstimado.toFixed(2)}</div>
              </div>
              <DialogFooter>
                <Button variant="outline" onClick={() => setConfirmOpen(false)}>Cancelar</Button>
                <Button className="bg-gradient-eco" onClick={async () => { setConfirmOpen(false); await handleSubmit(); }}>
                  Confirmar y crear
                </Button>
              </DialogFooter>
            </DialogContent>
